...
namespace NacidRas.Integrations.OaiPmhProvider.Providers
{
	/// <summary>
	...
	public class DataProvider
	{
		...
		public DataProvider(OaiConfiguration configuration,
		...
		private XDocument CreateGetRecord(DateTime date, ArgumentContainer arguments)
		{
			...
			var record = recordRepository.GetRecord(arguments.Identifier, arguments.MetadataPrefix);
		...
		private XDocument CreateListIdentifiersOrRecords(DateTime date, ArgumentContainer arguments, OaiVerb verb, ResumptionToken resumptionToken = null)
		{
			...
			var recordContainer = verb == OaiVerb.ListRecords ?
			...
			{
				recordContainer.ResumptionToken.ExpirationDate = DateTime.UtcNow.Add(configuration.ExpirationTimeSpan);
				content.Add(resumptionTokenConverter.ToXElement(recordContainer.ResumptionToken));
			}

			return CreateXml(date, root.ToArray());
		}

		private XDocument CreateListSets(DateTime date, ArgumentContainer arguments, ResumptionToken resumptionToken = null)
		{
			...
			var setContainer = setRepository.GetSets(arguments, resumptionToken);
			...
			if (setContainer != null)
			{
				...
				{
					setContainer.ResumptionToken.ExpirationDate = DateTime.UtcNow.Add(configuration.ExpirationTimeSpan);
					content.Add(resumptionTokenConverter.ToXElement(setContainer.ResumptionToken));
				}
			}
